...
import { getNextGreyYarnPoInvoiceNo } from "../queries/sequences.js";
...
async function createTaxGridDetails(connection, taxGridDetails, GTGRPBILLENTRYID) {
    const promises = taxGridDetails.map(async (tax) => {
        const taxCreate = `INSERT INTO gtgrpbillentrytaxdet 
        (GTGRPBILLENTRYTAXDETID,GTGRPBILLENTRYID,NOTES1,
        SF,TAX1,
        REGISTERVALUE,ADVALUE,ADTEMPRND,ADSUGGESTIVE,ADFORMULA,ADID,
        ADORDER, ADPORM,ADNAME,GTGRPBILLENTRYTAXDETROW)
        VALUES (supplierseq.nextVal,'${GTGRPBILLENTRYID}', '${tax.notes}',
        '${tax.sf}', '${tax.numId}', 
