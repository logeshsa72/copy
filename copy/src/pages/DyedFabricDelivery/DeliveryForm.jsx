...
import { useGetGreyYarnPoDeliveryDocIdQuery } from '../../redux/services/greyYarnPoDelivery';
import { getMonthValue } from '../../utils/date';
import { useAddGreyYarnPoDeliveryMutation, useGetGreyYarnPoDeliveryIdQuery, useUpdateGreyYarnPoDeliveryMutation } from '../../redux/services/greyYarnPoDelivery';
...
import { useGetGreyYarnPoQuery } from '../../redux/services/greyYarnPo';
import { IoSave } from 'react-icons/io5';
import { IoReturnDownBackSharp } from "react-icons/io5";
import { useAddDyedFabricPoDeliveryMutation, useGetDyedFabricPoDeliveryDocIdQuery, useGetDyedFabricPoDeliveryIdQuery, useUpdateDyedFabricPoDeliveryMutation } from '../../redux/services/dyedFabricPoDelivery';
import { useGetDyedFabricPoQuery } from '../../redux/services/dyedFabricPo';
const DeliveryForm = ({ setForm, selectedPos, setSelectedPos, delNo = null }) => {
  ...
  const [addData] = useAddDyedFabricPoDeliveryMutation()
  const [updateData] = useUpdateDyedFabricPoDeliveryMutation()
  const [yPoIno, setYPoIno] = useState('')
  const [yPoInDate, setYPoInDate] = useState(today)

  let gtCompMastId = localStorage.getItem('gtCompMastId')
  const { data: inDocId, isLoading: isDocLoading, isFetching: isDocFetching } = useGetDyedFabricPoDeliveryDocIdQuery({ skip: !delNo })
  useEffect(() => {
    if (delNo) return
  ...
  const { refetch } = useGetDyedFabricPoQuery({ params: { gtCompMastId, isAccepted: true, deliveryFilter: true } })

  const { data: singleData, isLoading, isFetching } = useGetDyedFabricPoDeliveryIdQuery({ fpino: delNo }, { skip: !delNo })

console.log(singleData,"singleData",delNo,"delll")

  useEffect(() => {
    const singleDelData = singleData?.data
    if (!singleDelData) return

    setYPoIno(singleDelData?.yPoIno ? singleDelData?.yPoIno : '')
    setYPoInDate(singleDelData?.yPoInDate ? singleDelData?.yPoInDate : today)
  ...
  const data = { remarks, supplierDcDate: moment(dcDate).format("DD-MM-YYYY"), supplierDcNo, deliveryDetails: deliveryDetails, supplierId: gtCompMastId, fabpurInwardNo: delNo }
  const handleSubmitCustom = async (callback, data, text) => {
    console.log(data,"frntend")
    try {
        ...
        <div className='flex border border-gray-400 gap-5'>
          <div className="w-full flex items-center" >
            <div className="flex justify-evenly w-full">
              {/* <div className=" flex  ">
                <div className=''> <lable className="font-semibold text-xs" >Delivery No : </lable><span className="font-light text-xs">{yPoIno}</span></div>

                <div>
                  <lable className="font-semibold text-xs" > Delivery Date : </lable> <span className="font-light text-xs">{getMonthValue(yPoInDate)}</span></div>
              </div> */}
              <div className='flex w-full justify-evenly'>
                <div className="flex items-center">
                  <lable className="font-semibold text-xs" >delivery No : </lable><span className="font-light text-xs">{yPoIno}</span>
                </div>
                <div><lable className="font-semibold text-xs" > delivery Date: </lable> <span className="font-light text-xs">{getMonthValue(yPoInDate)}</span><br /></div></div>
        ...
        <>
          ...
          <div className='   overflow-x-scroll'>
            <table className="border text-xs overflow-x-scroll w-full">
              ...
              <tbody className=' even:bg-white odd:bg-blue-100 '>{console.log(deliveryDetails,"deliveryDetails")}
                {(deliveryDetails?deliveryDetails:[]).map((item, index) => (
                  ...
                  <tr className={'even:bg-gray-300 odd:bg-white'} key={index}>
                    ...
                    <td className=" border-r text-left">{item.color}</td>
                     ...
                     <td className=" border-r ">{item.uom}</td>
                       <td className=" border-r ">{item.fDia}</td>
                       <td className=" border-r ">{item.kDia}</td>
                        <td className=" border-r ">{item.gg}</td>
                       <td className=" border-r ">{item.gsm}</td>
                        <td className=" border-r ">{item.ll}</td>
                        <td className=" border-r ">{item.fabricDesign}</td>
                        <td className=" border-r ">{item.fabType}</td>
                    ...
                    <td className="pl-3 text-right ">{(item?.poRate ? item?.poRate : 0).toFixed(2)}</td>
                    ...
                    <td className=" border-r  text-right">
                      <input className='table-input  text-right' value={item.grnRolls}
                        onChange={(e) => handleOnChange(e.target.value, 'grnRolls', index)} />
                    </td>
                    ...
                    <td className="pl-5 border-r  text-right">
                      <input className='table-input flex' value={item.grnQty} 
                      onChange={(e) => handleOnChange(e.target.value, 'grnQty', index)} />
                    </td>
