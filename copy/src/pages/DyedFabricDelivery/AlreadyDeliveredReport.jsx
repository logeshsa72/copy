...
import { useGetGreyYarnPoDeliveryQuery } from '../../redux/services/greyYarnPoDelivery';
import { useGetDyedFabricPoDeliveryQuery } from '../../redux/services/dyedFabricPoDelivery';
const AlreadyDeliveredReport = ({ setDocNo }) => {
  ...
  const { data: yarnPoDeliveryData } = useGetDyedFabricPoDeliveryQuery({ params: { gtCompMastId } })

  const handleOnclick = (e) => {
    setCurrentPageNumber(reactPaginateIndexToPageNumber(e.selected));
  }

  let deliveryItems = useMemo(() => yarnPoDeliveryData?.data ? yarnPoDeliveryData.data : [], [yarnPoDeliveryData])
    ...
    <div class="font-roboto p-1 border-solid border-2 border-gray-300 ">
      ...
      <div className='overflow-x-scroll'>
        <table class="  text-xs overflow-x-scroll w-full">
          ...
          <tbody className=''>{console.log(deliveryItems,"deliveryyy")}
            {(deliveryItems ? deliveryItems :[] ).map((item, index) => (
              <>
                <tr className={' even:bg-gray-300 odd:bg-white'}
                  key={index}
                  onClick={() => setDocNo(item?.fabpurInwardNo )}
                >
                  ...
                  <td className="py-[2px]whitespace-nowrap text-xs font-medium pl-1 text-left text-black ">
                    {item?.fabpurInwardNo ? item?.fabpurInwardNo :""}
                  </td>
                  {/* <td className="py-[2px]  ">
                    <span
                      className="py-[2px]  text-xs  text-black-800 "
                    >
                      {item?.delTo ? item.delTo :"---"}
                    </span>
                  </td> */}
                  <td className="py-[2px]  whitespace-nowrap text-xs text-left font-medium pl-1 ">
                    <div className="py-[2px] text-black">
                    </div>
                    {getMonthValue(item?.dFInwardDate)}
                  </td>
                  <td className="py-[2px]  whitespace-nowrap text-xs font-medium text-left pl-1  text-black-500 ">
                    {getMonthValue(item.dcDate)}
                  </td>
                  <td className="py-[2px]  whitespace-nowrap text-left text-xs font-medium pl-1 text-black ">
                    {item?.remarks}
                  </td>
                  {/* <td className="py-[2px]  whitespace-nowrap text-left text-xs font-medium  text-black ">
                    {item.vehicleNo}
                  </td> */}
                  <td className="py-[2px]  whitespace-nowra text-right text-xs font-medium pr-1  text-black ">
                    {item?.supplierDcNo}
                  </td>
            ...
            <>
              {Array.from({ length: 17 - (yarnPoDeliveryData?.data ? yarnPoDeliveryData.data : []).length }).map((_, index) =>
