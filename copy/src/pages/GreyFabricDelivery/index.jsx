...
const GreyFabricDelivery = () => {
    const [alreadyDelivered, setAlreadyDelivered] = useState(false);
    const [selectedPos, setSelectedPos] = useState([])
    let gtCompMastId = localStorage.getItem('gtCompMastId')

    const { refetch } = useGetGreyFabricPoDeliveryQuery({ params: { gtCompMastId } })
    const { refetch: fetch } = useGetGreyFabricPoQuery({ params: { gtCompMastId, isAccepted: true, deliveryFilter: true } })
    return (
        <div className='border-solid border-2 border-gray-300 '>
            ...
            <div className='flex justify-between   border-solid border-2 border-gray-300 m-1'>
                ...
                <h1 className='text-xl p--1   bg-clip-text text-orange-600 '>Grey Fabric Delivery</h1>
...
export default GreyFabricDelivery;
