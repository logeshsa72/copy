...
import { useGetGreyFabricPoItemsQuery } from '../../redux/services/greyFabricPo'
...
const SelectItems = ({ selectedPos, deliveryDetails, setDeliveryDetails, onModalClose }) => {
    ...
    const { data: poItems } = useGetGreyFabricPoItemsQuery({ filterPoList: JSON.stringify(selectedPos), deliveryFilter: true }, { skip: selectedPos.length === 0 })
                ...
                <tbody className=' even:bg-white odd:bg-blue-100 '>
                    {(poItemsData).map((item, index) => (
                        <tr className={"cursor-pointer"} key={index} onClick={() => handleChangeSelectPoItem(item)} >
                            ...
                            <td className=" border-r ">{item.gfPoInwardNo}</td>
