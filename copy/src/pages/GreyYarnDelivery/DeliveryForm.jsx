...
const DeliveryForm = ({ setForm, selectedPos, setSelectedPos, delNo = null }) => {
  ...
  const [addData] = useAddGreyYarnPoDeliveryMutation()
  const [updateData] = useUpdateGreyYarnPoDeliveryMutation()
  ...
  const { data: inDocId, isLoading: isDocLoading, isFetching: isDocFetching } = useGetGreyYarnPoDeliveryDocIdQuery({ skip: !delNo })
  useEffect(() => {
    if (delNo) return
  ...
  const { refetch } = useGetGreyYarnPoQuery({ params: { gtCompMastId, isAccepted: true, deliveryFilter: true } })

  const { data: singleData, isLoading, isFetching } = useGetGreyYarnPoDeliveryIdQuery({ ypoIno: delNo }, { skip: !delNo })
  ...
  const data = { remarks, supplierDcDate: moment(dcDate).format("DD-MM-YYYY"), supplierDcNo, vehicleNo, deliveryDetails: deliveryDetails, supplierId: gtCompMastId, yarnPoInwardNo: delNo }
