...
const DyedFabricPo = () => {
  ...
  const { data, refetch, isLoading, isFetching } = useGetDyedFabricPoQuery({ params: { gtCompMastId, isAccepted: showAcceptedPo } })
  ...
  return (
    <div class="font-roboto  border-solid border-2 border-gray-300  ">
      <div className='flex justify-between items-center m-1   border-solid border-2 border-gray-300 '>
        <h1 className='text-xl text-orange-600 '>DYED FABRIC PURCHASE ORDER</h1>
        <div className='flex justify-end'>

          <div className='flex gap-5 text-xs items-end m-2'>
            <div><button className={`${showAcceptedPo === undefined ? 'bg-green-400 text-white rounded p-1' : 'bg-gray-200 rounded p-1'}`} onClick={() => setShowAcceptedPo(undefined)}>Show All</button></div> 
             <div><button className={`${showAcceptedPo === true ? 'bg-green-400 text-white  rounded p-1' : 'bg-gray-200  rounded p-1'}`} onClick={() => setShowAcceptedPo(true)}>Accepted</button></div>
      ...
      <div className='overflow-x-scroll'>
        <table class=" border text-xs overflow-x-scroll w-screen ">
          ...
          <tbody className='table-auto'>{console.log(poData,"podtaaaa")}
              ...
              <>
                ...
                >
                  ...
                  <td className="  text-xs  text-center text-black-500   ">
                    {(item?.isAccepted ? <Tooltip placement="top" title="Accepted"> <button> <GiCheckMark className='text-xl text-green-500' /></button> </Tooltip>
                     : <Tooltip placement="top" title="Not Accepted"> <button ><FaCircleXmark className='hover:text-green-500 text-xl' onClick={() => handleStatus(item?.poNo)} /></button> </Tooltip>)}
                  </td>
