...
import { useGetAccessoryPoDetailsByIdQuery, useGetAccessoryPoQuery, useUpdateStatusMutation } from '../../redux/services/accessoryPo';
...
const AccessoryPo = () => {
  ...
  const { data, refetch, isLoading, isFetching } = useGetAccessoryPoQuery({ params: { gtCompMastId, isAccepted: showAcceptedPo } })
  ...
  const { data: poDetailsData } = useGetAccessoryPoDetailsByIdQuery({ poNo: po }, { skip: !po })
  ...
  return (
    <div class="font-roboto  border-solid border-2 border-gray-300  ">
      <div className='flex justify-between items-center m-1   border-solid border-2 border-gray-300 '>
        <h1 className='text-xl text-orange-600 '>ACCESSORY PURCHASE ORDER</h1>
                                  ...
                                  <tbody className=' even:bg-white odd:bg-blue-100'>
                                    {(poDetails).map((item, index) => (
                                      <tr className={index % 2 === 0 ? '' : 'bg-orange-100'} key={index}>
                                        <td className="p-2 border-r ">{item.accessory}</td>
                                        <td className="p-2 border-r ">{item.color}</td>
                                        <td className=" border-r ">{item.orderNo}</td>
                                        <td className="pl-5 border-r ">{item.poQty}</td>
                                        <td className="pl-2 border-r flex justify-end py-2">{item.porate ? parseFloat(item.porate).toFixed(2) : ''}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
