...
import { useGetDyedYarnPoDeliveryQuery } from '../../redux/services/dyedYarnPoDelivery';
const AlreadyDeliveredReport = ({ setDocNo }) => {
  ...
  const { data: yarnPoDeliveryData } = useGetDyedYarnPoDeliveryQuery({ params: { gtCompMastId } })
            ...
            {(deliveryItems).map((item, index) => (
              <>
                <tr className={' even:bg-gray-300 odd:bg-white'}
                  key={index}
                  onClick={() => setDocNo(item.dYarnPoInwardNo)}
                >
