...
import { useGetDyedYarnPoDeliveryDocIdQuery } from '../../redux/services/dyedYarnPoDelivery';
import { getMonthValue } from '../../utils/date';
import { useAddDyedYarnPoDeliveryMutation, useGetDyedYarnPoDeliveryIdQuery, useUpdateDyedYarnPoDeliveryMutation } from '../../redux/services/dyedYarnPoDelivery';
import { useNavigate } from 'react-router-dom';
import DelDtl from '../../components/DyedDeliveryDetails';
...
const DeliveryForm = ({ setForm, selectedPos, setSelectedPos, delNo = null }) => {
  ...
  const [addData] = useAddDyedYarnPoDeliveryMutation()
  const [updateData] = useUpdateDyedYarnPoDeliveryMutation()
  ...
  const { data: inDocId, isLoading: isDocLoading, isFetching: isDocFetching } = useGetDyedYarnPoDeliveryDocIdQuery({ skip: !delNo })
  useEffect(() => {
    if (delNo) return
  ...
  const { refetch } = useGetDyedYarnPoQuery({ params: { gtCompMastId, isAccepted: true, deliveryFilter: true } })

  const { data: singleData, isLoading, isFetching } = useGetDyedYarnPoDeliveryIdQuery({ ypoIno: delNo }, { skip: !delNo })
