...
const DyedYarnPo = () => {
  ...
  const { data, refetch, isLoading, isFetching } = useGetDyedYarnPoQuery({ params: { gtCompMastId, isAccepted: showAcceptedPo } })
  console.log(data?.data ? data.data : [], 'bill data');
  useEffect(() => {
  ...
  return (
    <div class="font-roboto  border-solid border-2 border-gray-300  ">
      <div className='flex justify-between items-center m-1   border-solid border-2 border-gray-300 '>
        <h1 className='text-xl text-orange-600 '>DYED YARN PURCHASE ORDER</h1>
              ...
              <>
                ...
                >
                  ...
                  <td className="  text-xs  text-center text-black-500   ">
                    {(item.isAccepted && JSON.parse(item.isAccepted) ? <Tooltip placement="top" title="Accepted"> <button> <GiCheckMark className='text-xl text-green-500' /></button> </Tooltip> : <Tooltip placement="top" title="Not Accepted"> <button ><FaCircleXmark className='hover:text-green-500 text-xl' onClick={() => handleStatus(item.poNo)} /></button> </Tooltip>)}
                  </td>
