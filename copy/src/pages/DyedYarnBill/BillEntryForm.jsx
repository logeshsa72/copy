...
import { useAddDyedYarnPoBillEntryMutation, useGetDyedYarnPoBillEntryDocIdQuery, useGetDyedYarnPoBillEntryIdQuery, useUpdateDyedYarnPoBillEntryMutation } from '../../redux/services/dyedYarnBillEntry';
...
const BillForm = ({ setForm, selectedPos, setSelectedPos, delNo = null }) => {
    ...
    const [addData] = useAddDyedYarnPoBillEntryMutation()
    const [updateData] = useUpdateDyedYarnPoBillEntryMutation()
    ...
    const { data: inDocId, isLoading: isDocLoading, isFetching: isDocFetching } = useGetDyedYarnPoBillEntryDocIdQuery({ skip: !delNo })
    useEffect(() => {
    ...
    const { data: singleData, isLoading, isFetching } = useGetDyedYarnPoBillEntryIdQuery({ billEntryNo: delNo }, { skip: !delNo })
    useEffect(() => {
                                ...
                                {(invoiceDetails).map((item, index) => (
                                    <tr className={index % 2 === 0 ? '' : 'bg-gray-300'} key={index}>
                                        ...
                                        <td className=" border-r pl-2 text-right">
                                            <input className=' table-input text-right' value={item.billQty} 
                                            onChange={(e) => handleOnChange(e.target.value, 'billQty', index)} />
                                        </td>
